    ...
    try {
        ...
        tokenHandler._token.verify(token, userId, async (ok) => {
            ...
            const doc = new PipelineTemplate({
                ...
                useDocker: body?.useDocker || false,
                dockerImage: body?.dockerImage,
                dockerfile: body?.dockerfile,
                dockerBuildArgs: body?.dockerBuildArgs || [],
                autoGenerateDockerfile: body?.autoGenerateDockerfile || false,
                createdBy: userId,
            });
            await doc.save();
...
handler._impl.delete = async (req, callback) => {
    try {
        const { headersObject, queryStringObject } = req;
        const token = typeof headersObject?.token === 'string' ? headersObject.token : false;
        const id = typeof queryStringObject?.id === 'string' ? queryStringObject.id : null;

        if (!id) return callback(400, { error: 'id required' });

        // If token provided, verify it (optional auth for now)
        if (token) {
            // Try to find the pipeline to get the creator
            const pipeline = await PipelineTemplate.findById(id);
            if (pipeline && pipeline.createdBy) {
                const userId = pipeline.createdBy;
                return tokenHandler._token.verify(token, userId, async (ok) => {
                    if (!ok) return callback(403, { error: 'Authentication failed.' });
                    await PipelineTemplate.findByIdAndDelete(id);
                    callback(200, { data: { message: 'Pipeline template deleted successfully' } });
                });
            }
        }

        // Delete without auth check (fallback for backward compatibility)
        const result = await PipelineTemplate.findByIdAndDelete(id);
        if (!result) return callback(404, { error: 'Pipeline template not found' });
        callback(200, { data: { message: 'Pipeline template deleted successfully' } });
    } catch (e) {
